...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($apellido) || empty($direccion) || empty($email) || empty($telefono) || empty($password)) {
    ...
    try {
        // Insertar cliente
        $sql_cliente = "INSERT INTO clientes (nombre, apellido, direccion, email, telefono, password) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt_cliente = $conexion->prepare($sql_cliente);
        $stmt_cliente->bind_param('ssssss', $nombre, $apellido, $direccion, $email, $telefono, $password);

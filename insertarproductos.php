...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['nombre'], $_POST['descripcion'], $_POST['precio'], $_POST['categoria'])) {
    ...
    $imagen = "";
    $disponible = true; // Marcar el producto como disponible por defecto
    ...
    $sql = "INSERT INTO productos (nombre, descripcion, precio, id_categoria, imagen, disponible) VALUES (?, ?, ?, ?, ?, ?)";
    if ($stmt = $conexion->prepare($sql)) {
        $stmt->bind_param("ssdssi", $nombre, $descripcion, $precio, $categoria, $imagen, $disponible);
        if ($stmt->execute()) {

...
<style>
    .escudo {
        ...
        animation: escudoAnim 2s ease-in-out infinite;
    }
    .escudo img{
        width: 100%;
        height: 100%;
        object-fit: cover;
        animation: shadowAnim 1s ease-in-out infinite;
...
function mostrarNotificacionCarrito(cantidad) {
    ...
    // Si la cantidad es mayor a 0, se muestra la notificación, sino se oculta
    if (cantidad > 0) {
        notificacion.classList.add("mostrar");
        campanita.classList.add("sacudir");
    } else {
        notificacion.classList.remove("mostrar");
        campanita.classList.remove("sacudir");
    }
...
function agregarAlCarrito() {
    // Se obtiene la cantidad actual del carrito o 0 si no existe
    let cantidad = parseInt(document.getElementById("cantidadCarrito").textContent) || 0;

    // Incrementa la cantidad en 1
    cantidad += 1;

    // Muestra la notificación con la cantidad actualizada
    mostrarNotificacionCarrito(cantidad);
}

// Llamada de ejemplo para agregar un producto al carrito
document.getElementById("botonAgregarCarrito").addEventListener("click", agregarAlCarrito); 
                ...
                tarjeta.innerHTML = `
                    <div class="card product-card">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text"><strong>Precio: ${precio} XAF</strong></p>
